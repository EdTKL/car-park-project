...
export const detect2 = async (
  ...
  try {
    ...
    if (needsReinit) {
      ...
      console.log("cam2 reinitialized");
    }
      ...
      setTimeout(async () => {
        ...
        try {
          const res = await fetch(
            "https://api.carpark.live/carpark/outRecord",
            {
              method: "POST",
 ...
 * @param {HTMLCanvasElement} canvasRef canvas reference
 */
export const detectVideo2 = (vidSource, model, canvasRef, staffID, dispatch) => {
  /**
   * Function to detect every frame from video
   */
  const detectFrame2 = async () => {
    ...
    detect2(vidSource, model, canvasRef, staffID, dispatch, () => {
      setTimeout(() => {
        requestAnimationFrame(detectFrame2);
      }, 1000); // get another frame
    });
  };

  detectFrame2(); // initialize to detect every frame
};
